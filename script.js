...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTable(owner, repo, branch) {
    ...
    for (const it of rows) {
      ...
      rowDiv.onclick = (e) => {
        e.preventDefault();
        if (it.type === "dir") {
          navigate(joinPath(state.path, it.name));
        } else {
          showPreview(owner, repo, branch, joinPath(state.path, it.name));
        }
      };
  ...
  async function fetchContents(o, r, p, b) {
    const url = api(o, r, p, b);
    setStatus("warn", "Loading " + (p || "/") + " …");
    setNotice("");
    try {
      ...
      if (!Array.isArray(data)) {
        setNotice("This path is a file.");
        state.entries = [];
        renderTable(o, r, b);
        return;
      }
      state.entries = data.map((x) => ({
        name: x.name,
        path: x.path,
        type: x.type,
        size: x.size ?? null,
  ...
  function navigate(path) {
    state.path = path || "";
    location.hash = encodeURIComponent(state.path);
    doLoad();
  }

  function doLoad() {
    const { owner, repo, branch } = DEFAULTS;
    fetchContents(owner, repo, state.path, branch);
  }
